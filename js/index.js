...
var gameStatus = "play";
var cameraPosGame = 160; // 视角远近
var cameraPosGameOver = 260; // 游戏结束时的视角距离
...
Monster = function () {
  ...
  tailGeom.applyMatrix(new THREE.Matrix4().makeRotationX(-PI / 2));
  tailGeom.applyMatrix(new THREE.Matrix4().makeRotationZ(PI / 4));
...
Monster.prototype.run = function () {
  ...
  this.tail.rotation.x = .2 + Math.sin(t - PI / 2);
...
Hero.prototype.hang = function () {
  ...
  TweenMax.to(this.head.rotation, sp, {
    x: PI / 6, ease: ease, onComplete: function () {
      _this.nod();
    }
  });

  TweenMax.to(this.earL.rotation, sp, {x: PI / 3, ease: ease});
  TweenMax.to(this.earR.rotation, sp, {x: PI / 3, ease: ease});
...
Monster.prototype.nod = function () {
  ...
  // HEAD
  var tHeadRotY = -PI / 3 + Math.random() * .5;
  var tHeadRotX = PI / 3 - .2 + Math.random() * .4;
  TweenMax.to(this.head.rotation, sp, {
...
Monster.prototype.sit = function () {
  ...
  TweenMax.to(this.head.rotation, sp, {x: PI / 3, y: -PI / 3, ease: ease});
  TweenMax.to(this.tail.rotation, sp, {x: 2, y: PI / 4, ease: ease});
  ...
  //TweenMax.to(this.body.rotation, sp, {y:PI/4});
...
function getMalus() {
  ...
  TweenMax.to(obstacle.mesh.rotation, 4, {
    x: PI * 3, z: PI * 3, y: PI * 6, ease: Power4.easeOut, onComplete: function () {
...
function resetGame() {
  ...
  audio.play();
  updateLevel();
  levelInterval = setInterval(updateLevel, CONFIG.levelUpdateFreq);
}
